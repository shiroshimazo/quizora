...
public class resultsController {
 ...
 protected List<ResultRecord> loadResults(quizora.auth.AuthenticatedUser identity)throws java.sql.SQLException{return new ResultsDAO().load(identity);}
 protected String accessMessage(){return "Administrator access is required. Sign in again.";}
 @FXML public void refresh(){if(busy)return;busy=true;clear();controls();resultsMessage.setText("Loading results...");var admin=UserSession.current();Task<List<ResultRecord>> task=new Task<>(){protected List<ResultRecord> call()throws Exception{return loadResults(admin);}};
  task.setOnSucceeded(e->{busy=false;if(UserSession.current()!=admin){clear();resultsMessage.setText("Your session changed. Sign in again.");}else display(task.getValue());controls();});task.setOnFailed(e->{busy=false;clear();controls();if(UserSession.current()!=admin){resultsMessage.setText("Your session changed. Sign in again.");return;}resultsMessage.setText(task.getException() instanceof SecurityException?accessMessage():"Results unavailable. Check the database connection and refresh.");});Thread t=new Thread(task,"quizora-results");t.setDaemon(true);t.start();
 }

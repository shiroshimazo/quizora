...
import quizora.DAO.AdminDashboardDAO;
import quizora.auth.UserSession;
import quizora.model.AdminDashboardData;

public class adminDashboardController implements Initializable {
    @FXML private Label studentsValue, teachersValue, quizzesValue, subjectsValue, submissionsValue, averageValue;
    ...
    private Task<AdminDashboardData> loading;
    ...
    private void refresh() {
        ...
        if (identity == null || !"admin".equals(identity.role())) {
            clear();
            statusLabel.setText("Sign in as an administrator to view system information.");
        ...
        loading = new Task<>() {
            @Override protected AdminDashboardData call() throws Exception {
                return new AdminDashboardDAO().load(identity);
            }
        };
        loading.setOnSucceeded(event -> {
            if (UserSession.current() != identity) { clear(); return; }
        ...
        loading.setOnFailed(event -> {
            clear();
            refreshButton.setDisable(false);
            statusLabel.setText("Overview unavailable. Check your connection and admin access, then refresh.");
        });
        Thread worker = new Thread(loading, "quizora-admin-overview");
        worker.setDaemon(true);
        worker.start();
    }

    private void clear() {
        for (Label label : new Label[]{studentsValue, teachersValue, quizzesValue, subjectsValue,
    ...
    void render(AdminDashboardData data) {
        NumberFormat integer = NumberFormat.getIntegerInstance();
        studentsValue.setText(integer.format(data.students()));
        teachersValue.setText(integer.format(data.teachers()));
        ...
        configureCounts((NumberAxis) subjectChart.getYAxis(),
                data.quizzesBySubject().stream().mapToLong(AdminDashboardData.Count::value).max().orElse(0));
        ...
        configureCounts((NumberAxis) submissionChart.getYAxis(),
                data.submissionsByDay().stream().mapToLong(AdminDashboardData.DailyCount::value).max().orElse(0));

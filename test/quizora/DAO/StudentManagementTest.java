...
public class StudentManagementTest {
    public static void main(String[] args) throws Exception {
        ...
        try (var connection = databaseConnection.getConnection()) {
            try {
                ...
                StudentRecord original=find(dao,admin,studentId);
                ...
                try { new StudentChanges("","","bad",true); throw new AssertionError("Invalid edit accepted"); }
                catch(IllegalArgumentException expected) { }
                try { dao.edit(admin,original,new StudentChanges("Student","admin",tag+"@example.invalid",true)); throw new AssertionError("Duplicate accepted"); }
                catch(SQLException expected) { require(expected.getErrorCode()==1062,"Duplicate validation"); }
                StudentRecord edited=dao.edit(admin,original,new StudentChanges("Edited Student","edited-"+tag,tag+"@example.invalid",false));
                ...
                edited=dao.edit(admin,edited,new StudentChanges(edited.name(),edited.username(),edited.email(),true));
                require(auth.authenticate(edited.email(),"Test@123".toCharArray()).isPresent(),"Reactivated login and preserved password");
                StudentRecord archived=dao.archive(admin,edited);
                ...
                try { dao.edit(admin,archived,new StudentChanges("Changed",archived.username(),archived.email(),true));
    ...
    private static StudentRecord find(StudentManagementDAO dao,AuthenticatedUser admin,long id) throws SQLException {

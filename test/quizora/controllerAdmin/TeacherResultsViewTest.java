...
public class TeacherResultsViewTest {
 ...
 @SuppressWarnings("unchecked") public static void main(String[]args)throws Exception{
  ...
  try{fx(()->{
   quizora.Quizora.satoshi(14);FXMLLoader loader=new FXMLLoader(TeacherResultsViewTest.class.getResource("/Resources/fxml/teacher/studentResults.fxml"));ScrollPane root=loader.load();resultsController controller=loader.getController();root.setVisible(true);root.setManaged(true);Stage stage=new Stage();Scene scene=new Scene(root,1100,850);scene.getStylesheets().add(TeacherResultsViewTest.class.getResource("/Resources/css/dashboard.css").toExternalForm());stage.setScene(scene);stage.show();
   try{
    ...
    require(controller instanceof quizora.controllerTeacher.studentResultsController,"Teacher results controller");
    controller.display(rows);
    ((Label)root.lookup("#resultsMessage")).setText("Illustrative test data ? not database records");
    ...
    ((Label)root.lookup("#resultsMessage")).setText("Illustrative test data - not database records");
    javax.imageio.ImageIO.write(javafx.embed.swing.SwingFXUtils.fromFXImage(root.snapshot(null,null),null),"png",new java.io.File("build/teacher-results-preview.png"));

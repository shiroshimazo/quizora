...
public class TeacherProfileTest {
 ...
 public static void main(String[] args)throws Exception{
  ...
  try{
   ...
   fx(()->{UserSession.signIn(admin);stage=new Stage();PanelRouter.open(stage);((ToggleButton)stage.getScene().lookup("#profileButton")).fire();return null;});await(()->((Label)stage.getScene().lookup("#accountMessage")).getText().equals("Your profile is up to date."));
   ...
   fx(()->{if(!((Label)stage.getScene().lookup("#profileName")).getText().equals("Updated UI Profile"))throw new AssertionError("Display not updated");if(!UserSession.current().role().equals("teacher"))throw new AssertionError("Teacher role not preserved");if(!UserSession.current().fullName().equals("Updated UI Profile"))throw new AssertionError("Session name not updated");return null;});
   Platform.runLater(()->((Button)stage.getScene().lookup("#editProfileButton")).fire());await(()->dialog("Edit profile")!=null);
   ...
   accountManagementController pictureController=fx(()->{
    var loader=new javafx.fxml.FXMLLoader(TeacherProfileTest.class.getResource("/Resources/fxml/teacher/profile.fxml"));
   ...
   var accessIdentity=UserSession.current();var accessProfile=dao.load(accessIdentity);
   for(String state:List.of("is_active=FALSE","is_active=TRUE,archived_at=CURRENT_TIMESTAMP","archived_at=NULL,role='student'")) {
    try(var c=databaseConnection.getConnection();var statement=c.prepareStatement("UPDATE users SET "+state+" WHERE user_id=?")){statement.setLong(1,id);statement.executeUpdate();}
    try{dao.load(accessIdentity);throw new AssertionError("Unavailable teacher read allowed");}catch(SecurityException expected){}
    try{dao.save(accessIdentity,accessProfile,changes);throw new AssertionError("Unavailable teacher write allowed");}catch(SecurityException expected){}
   }
   System.out.println("PASS: own-teacher access, duplicates, stale updates, image validation/persistence, preserved login, profile modal validation/save/cancel, session name and responsive layout.");
